...
int fataddfiles(char *dirname, LPVOID memory, unsigned long stclst, char *buf)
{
    ...
    while (nl)
    {
        ...
        if (Form1->fatdeb->Checked)
            deb("F #%-3d %25s %8u байт (%.2f MB) ", nfiles, filename, fsize,
...
unsigned long ftppasvread(char *host, int port, char *buf, int bufsiz, unsigned long len)
{
    ...
    while ((ret = connect(tempsock, (SOCKADDR*)&clientService, sizeof(clientService))) == -1)
    {
        ...
        // ftpdeb("ftppasvread: невозможно соединица с сервером %s:%d\n\r  %s (%d)", host, port,
        // fmterr(err), err);
    ...
    while (true)
    {
        ...
        if (ss>0)
        {
            ftpdeb("    pasv read %d - %4lu мс", ss, GetTickCount()-tms);
            lastpasvread = ss;
        }
    ...
    // if (Form1->netdebug->Checked)
    tms = GetTickCount()-tms;
    ftpdeb("   ftppasvread() = read %d, numr: %lu ss %d, %d мс", readok, numr, ss, tms);
...
bool ftpconn(bool quick = false)
{
    ...
    Form1->knotts->Title->Text->Text = "соединение ...";
    shutdown(FtpSocket, SD_BOTH);
    closesocket(FtpSocket);
    ...
    deb("соединение с сервером %s (%s) ...", host, ip);
    // Form1->Canvas->Font->Color=clRed;
    // Form1->Canvas->Font->Size=12;
    // Form1->deblog->Canvas->TextOutA(Form1->Left+Form1->deblog->Width/2,Form1->deblog->Height/2,"соединение ...");
    DWORD ms = GetTickCount();
    ftpblocking(true);
    Form1->Button14->Caption = "отключить";
    ...
    {
        int err = WSAGetLastError();
        deb("Client: невозможно соединица с сервером %s\n\r  %s", host, fmterr(err));
    ...
    strcpy(ip, inet_ntoa(sa.sin_addr));
    ftplport = lport;
    ftpdeb("  соединен %u мс, laddr=%s:%d", ms, inet_ntoa(sa.sin_addr), lport);
    ...
    // Application->ProcessMessages();
    // ftpcmd("STaT", 0, 0);
    Form1->knotts->Title->Text->Text = "определение задержки ...";
    ...
    while (ftprl(buf, timeoutst, (char*)0)>0)
        continue;
    Form1->knotts->Title->Text->Text = "пароль ...";
    ...
    while (ftprl(buf, timeoutst*2, (char*)0)>0)
        ...
        {
            Form1->knotts->Title->Text->Text = buf;
            deb("   ошибка при аутентификации");
            ftpdeb(" > ошибка аутентификации");
            closesocket(FtpSocket);
            FtpSocket = 0;
    ...
    if (!quick)
    {
        ...
        {
            Form1->knotts->Title->Text->Text = ftpcmds[i]; // "инициализация %s ...";
            deb("  # %s", ftpcmds[i]);
        ...
        sprintf(buf, "LIST -l");
        ftpcmd(buf, 0, 0);
        Form1->knotts->Title->Text->Text = "чтение списка файлов ...";
        deb("чтение списка файлов ...");
    ...
    for (vector<CACHE>::iterator it = cache.begin();it!=cache.end();it++)
    {
        if ((*it).offset == offset)
        {
            if (len > (*it).len)
            {
                char str[1024];
                sprintf(str,
                    "  * КЕШ: не хватает данных %d @ %d : \r\n  нужно %d > есть %d, освобождаем",
...
unsigned long ftpgetfile(char *fname, unsigned long offset, char *buf, unsigned long len)
{
    ...
    // {
    // ftpdeb(" ! RST: фтп не отвечает на %s", lastftpcmd);
    // shutdown(FtpSocket, SD_BOTH);
    // closesocket(FtpSocket);
    // FtpSocket = 0;
    // ftpconn(true);
    ...
    IN LARGE_INTEGER ByteOffset, IN BOOL PagingIo, OUT PULONG BytesRead)
{
    ...
    *BytesRead = (unsigned long)Length;
    ...
    // tms = GetTickCount()-tms;
    // if (Form1->debcheck->Checked)
    // deb("#%3d READ  %5lu @ %-8I64d CRC=0x%04X [%4d мс]", numpkt++, Length,
    ...
    // tms = GetTickCount()-tms;
    // deb("#%3d READ  %5lu @ %-8I64d CRC=0x%04X ппд %d [%d мс]", numpkt, Length,
    // ByteOffset.QuadPart, checksum((unsigned short*)Buffer, Length), hits, tms);
    //
    // sprintf(str, "<ожидание>", 0, 1);
    ...
    // // tmsleft=15;
    //
    // sprintf(delaystr, "задержка %.2f %% (gt %.2f bt %.2f)", dp, gt, bt);
    //
    // if (dp>50.0)
    ...
    // if (hits == -1)
    // return IO_OP_STATUS_UNSUCCESSFUL;
    // *BytesRead = Length; // это обязательно!
    // return IO_OP_STATUS_SUCCESS;
    // }
    ...
    // char strc[128];
    // if (!upds)
    // sprintf(strc, "Закешировано", upds);
    // else
    // sprintf(strc, "%d ппд.", hits);
    // tms = GetTickCount()-tms;
    // deb("#%3d READ  %5lu @ %-8I64d CRC=0x%04X %s [%4d мс]", numpkt++, Length, ByteOffset.QuadPart,
    ...
    IN LARGE_INTEGER ByteOffset, IN BOOL PagingIo, OUT PULONG BytesWritten)
{
    ...
    *BytesWritten = (unsigned long)Length;
    ...
    if (ramdisk)
    {
        ...
        sprintf(str, "#%3d WRITE %5u @ %-8I64d CRC=0x%04X [%4d мс]", numpkt++, Length,
            ByteOffset.QuadPart, checksum((unsigned short*)Buffer, Length), tms);
        OutputDebugString(str);
        if (Form1->debcheck->Checked)
            deb(str);
    ...
    *BytesWritten = Length;
    ...
    // return IO_OP_STATUS_UNSUCCESSFUL;
    // }
    // *BytesWritten = Length; // это обязательно!
    //
...
VOID __stdcall OnEvent(IN HANDLE hDisk, IN LONG DiskEvent, IN PVOID Context)
{
    ...
    MessageBox(Application->MainFormHandle, "Критическая ошибка диска. Перезапуск.", "ошибка",
        MB_OK);
...
void __fastcall TForm1::Button5Click(TObject *Sender)
{
    if (ramdisk)
    {
        deb("   СRC RAM диска = 0x%04X memory @ %p", checksum((unsigned short*)memory,
                curdisksize), memory);
    ...
    pkt.cmd = 5;
    pkt.sign = 'A';
    deb(" * подщитываем СRC на сервере ...");
    Application->ProcessMessages();
    DWORD ms = GetTickCount();
    cchs->Enter();
...
void __fastcall TForm1::cacheblocksDblClick(TObject *Sender)
{
    ...
    int num = 0;
    ccs->Enter();
    fprintf(f, "   * Кеш - %d записей, %u байт (%u Мб):\r\n", cache.size(), totcachsize,
        totcachsize/1024/1024);

    fprintf(f,
        "  №   размер  смещение     обнвл. ппд.   время   прио\r\n-----------------------------------------------------------\r\n");
...
void __fastcall TForm1::FormPaint(TObject *Sender)
{
    ...
    try
    {
        ...
        if ((GetTickCount()-tms)>20)
        {
            ...
            sprintf(ss, "%d файл в %.1f Mб из %.1f Гб", files.size(),
                (double)((double)curdisksize/1024.0/1024.0),
                (double)((double)disksize/1024.0/1024.0/1024.0));
            //
            Label13->Caption = ss;
            sprintf(ss, "использовано %.2f MB из %.2f MB", (double)maxquad/1024/1024,
...
void __fastcall TForm1::makefatdisk(TObject *Sender)
{
    ...
    if (fatsize > curdisksize)
    {
        ...
        deb("фат  %d МБ @ %p", ns/1024/1024, memory);
    ...
    // deb("память для фат 2 таблицы %d МБ", ns/1024/1024);
    // memory = HeapReAlloc(GetProcessHeap(), 0, memory, ns);
    // curdisksize = ns;
    //
    // }
...
void __fastcall TForm1::Button14Click(TObject *Sender)
{
    ...
    if ((memory && curdisksize) || FtpSocket)
    {
        ...
        files.clear();
        fatfiles[0] = 0;
        Form1->Button14->Caption = "подключить";
